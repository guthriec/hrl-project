...
class MazeEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._disable_walls = disable_walls
        self._disable_movable_blocks = disable_movable_blocks
        self._force_flat = force_flat

        model_cls = self.__class__.MODEL_CLASS
        if model_cls is None:
            raise ValueError("MODEL_CLASS unspecified!")
        # Resolve XML relative to this file's directory to avoid CWD issues.
        xml_path = os.path.join(os.path.dirname(__file__), MODEL_DIR, model_cls.FILE)
        ...
        if self._disable_movable_blocks:
            self.blocks = False
        ...
        if self.blocks:
            ...
            geom_default = default.find(".//geom") if default is not None else None
            if geom_default is not None:
                geom_default.set("solimp", ".995 .995 .01")
        ...
        for geom in geoms:
            if "name" not in geom.attrib:
                raise ValueError("Every geom of the torso must have a name defined")

        # Write to a temp XML file and keep track for cleanup.
        fd, file_path = tempfile.mkstemp(text=True, suffix=".xml")
        os.close(
            fd
        )  # Avoid leaking the file descriptor; ElementTree will reopen the path.
    ...
    def reset(self, *args, **kwargs):
        # Normalize Gymnasium reset signature: returns (obs, info)
        self.t = 0
        inner = self.wrapped_env.reset(*args, **kwargs)
    ...
    def close(self):
        try:
            if hasattr(self, "wrapped_env") and self.wrapped_env is not None:
                self.wrapped_env.close()
        finally:
            if self._temp_xml_path and os.path.exists(self._temp_xml_path):
                try:
